...
function createHtml(layout) {
	...
	var htmlfoot = '<div id="browser"></div><script>window.onload = function () {var browser = document.getElementById("browser");browser.addEventListener("click", function () {this.style.display = "none";});function is_weixn_qq() {var ua = navigator.userAgent.toLowerCase();if (ua.match(/MicroMessenger/i) == "micromessenger") {return true;}return false;}var spanList = document.querySelectorAll("span[data-hasevent]");for (var i = 0; i < spanList.length; i++) {var item = spanList[i];item.addEventListener("click", function (e) {return (function (item) {if (is_weixn_qq()) {browser.style.display = "block";return;}var id1 = item.getAttribute("data-eventid1");var id2 = item.getAttribute("data-eventid2");id = id1;if (id2) {id = id1 + "/" + id2;}window.location.href = "tticarstorecall://" + id;})(item)})}}</script></body></html>';
