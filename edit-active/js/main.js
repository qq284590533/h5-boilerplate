...
function createHtml(layout) {
	...
	var htmlfoot = '<script>window.onload = function () {var spanList = document.querySelectorAll("span[data-hasevent]");for(var i=0; i<spanList.length; i++){var item = spanList[i];item.addEventListener("click", function (e) {return (function (item) {var id1 = item.getAttribute("data-eventid1");var id2 = item.getAttribute("data-eventid2");id = id1;if (id2) {id = id1 + "/" + id2;}window.location.href = "tticarstorecall://" + id;})(item)})}}</script></body></html>'
